...
| **E-003** | 3 放开编码器端到端（60 轮，编码器 lr 1e-4） | **18.93 ± 0.58** | 50.15 | ✅ 第 3 行，bootstrap 2/3 seed 显著；beam4 20.17 |
| E-004 | 留一手语者 E（泛化实验，不进消融表） | E-600 **7.64**（见过时 17.6） | 32.58 | ✅ 陌生手语者保留约 43%；D-026 结论 |
...
|---|---|---|
| 09-17 | ✅ 链路完成，四行表定稿、文档已写 | ✅ 推理封装 / FastAPI / 页面已完成；demo 切到 E-003 s3456 |
| 09-18 | 空闲；可选：关键点增广对 E-004 的效果 | agent LLM 重排 + 两行评测（需 API key） |
...
- **D-026 结论**：E-003 编码器 lr 1e-4，18.93 ± 0.58，+2.14；E-004 陌生人 7.64 vs 见过 17.6，保留 43%，摄像头模式保留并明示。
- **D-026**：阶段 3 = 放开编码器（单变量：编码器是否更新）。E-004 留一手语者 E，在 E 的全部 600 条上评，
  ...
  **数据结构已核实**：train 4,967/4,973 句唯一，dev/test 全唯一，官方划分天然句子不相交；"同一套 600 句每人打一遍"是错的，已改。
